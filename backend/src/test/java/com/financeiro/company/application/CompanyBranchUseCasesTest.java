...
class CompanyBranchUseCasesTest {
  @Test
  void createsGetsAndListsCompanies() {
  ...
  @Test
  void createsBranchOnlyForExistingCompany() {
    var companies = new Companies();
  ...
  @Test
  void branchLookupIsAlwaysCompanyScoped() {
    var branches = new Branches();
    branches.save(Branch.create(1L, "Main"));
    assertThat(new GetBranch(branches).execute(1L, 1L).name()).isEqualTo("Main");
    assertThatThrownBy(() -> new GetBranch(branches).execute(2L, 1L))
        .isInstanceOf(BranchNotFoundException.class);
    assertThatThrownBy(() -> new GetBranch(branches).execute(1L, 999L))
        .isInstanceOf(BranchNotFoundException.class);
  }

  @Test
  void listsOnlyBranchesFromTheExistingCompany() {
    var companies = new Companies();
    var branches = new Branches();
    ...
    var query = new PageQuery(0, 20, PageQuery.SortField.ID, PageQuery.SortDirection.ASC);
    PageResult<Branch> result =
        new ListBranchesByCompany(companies, branches).execute(one.id(), query);
  ...
  @Test
  void listBranchesDistinguishesMissingCompanyFromEmptyPage() {
    var companies = new Companies();
    var branches = new Branches();
    Company company = companies.save(Company.create("Acme"));
    var query = new PageQuery(0, 20, PageQuery.SortField.ID, PageQuery.SortDirection.ASC);
    assertThat(new ListBranchesByCompany(companies, branches).execute(company.id(), query).data())
        .isEmpty();
    assertThatThrownBy(() -> new ListBranchesByCompany(companies, branches).execute(999L, query))
        .isInstanceOf(CompanyNotFoundException.class);
  }
  ...
  void validatesPagination() {
    assertThatThrownBy(
            () -> new PageQuery(-1, 20, PageQuery.SortField.ID, PageQuery.SortDirection.ASC))
        .isInstanceOf(InvalidPageRequestException.class);
    assertThatThrownBy(
            () -> new PageQuery(0, 101, PageQuery.SortField.ID, PageQuery.SortDirection.ASC))
        .isInstanceOf(InvalidPageRequestException.class);
  }

  static final class Companies implements CompanyRepository {
    final Map<Long, Company> values = new LinkedHashMap<>();
    long sequence;

    public Company save(Company c) {
      var saved = Company.rehydrate(++sequence, c.name());
      values.put(sequence, saved);
      return saved;
    }
    ...
    public PageResult<Company> findPage(PageQuery q) {
  ...
  static final class Branches implements BranchRepository {
    final Map<Long, Branch> values = new LinkedHashMap<>();
    long sequence;

    public Branch save(Branch b) {
      var saved = Branch.rehydrate(++sequence, b.companyId(), b.name());
      values.put(sequence, saved);
      return saved;
    }

    public Optional<Branch> findByCompanyIdAndId(Long c, Long id) {
      return Optional.ofNullable(values.get(id)).filter(b -> b.companyId().equals(c));
    }

    public PageResult<Branch> findPageByCompanyId(Long c, PageQuery q) {
      var data = values.values().stream().filter(b -> b.companyId().equals(c)).toList();
      return new PageResult<>(data, q.page(), q.size(), data.size(), data.isEmpty() ? 0 : 1);
    }
  }
}
