...
class GlobalExceptionHandlerTest {
  ...
  @Test
  void mapsUnexpectedExceptionToSafeInternalErrorWithRequestTraceId(CapturedOutput output)
      throws Exception {
    MvcResult result =
        mockMvc
  ...
  void returnsIso8601TimestampAndRequestTraceId() throws Exception {
    MvcResult result =
        mockMvc.perform(get("/test/errors/not-found")).andExpect(status().isNotFound()).andReturn();
  ...
  private void assertTechnicalError(String endpoint, int expectedStatus, String code)
      throws Exception {
    mockMvc
        .perform(get("/test/errors/{endpoint}", endpoint))
  ...
  static class TestController {
    ...
    void technicalError(@PathVariable String type) {
      throw switch (type) {
        case "not-found" ->
            new ApiErrorException(ApiErrorType.RESOURCE_NOT_FOUND, "Resource not found.");
